(() => {
  ...
  const steps = [
    { title: '1. Choose the real outcome', body: 'Start with the problem you need to solve: verify an AI answer, preserve context, compare outputs, control an agent or make a decision.', target: 'purposeBtn', action: 'Choose a purpose' },
  ...
  function applyMode(value, announce = false) {
    ...
    if (button) { button.setAttribute('aria-pressed', String(enabled)); button.textContent = enabled ? 'Standard view' : 'Simple view'; button.title = enabled ? 'Return to the full interface' : 'Use larger text and fewer choices'; }
  ...
  function renderGuide() {
    ...
    $('simpleGuideStep').textContent = `${step + 1} of ${steps.length}`; $('simpleGuideTitle').textContent = current.title; $('simpleGuideBody').textContent = current.body; $('simpleGuideAction').textContent = current.action; $('simpleGuidePrev').disabled = step === 0; $('simpleGuideNext').textContent = step === steps.length - 1 ? 'Done' : 'Next';
    clearHighlight();
    const target = targetElement(current.target);
    if (target) { target.classList.add('simpleGuideTarget'); target.scrollIntoView({ block: 'nearest', inline: 'nearest', behavior: enabled ? 'auto' : 'smooth' }); }
  }
