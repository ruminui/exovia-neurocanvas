...
add('reproducible_install_lock', await exists('package-lock.json') ? 'PASS' : 'BLOCKED', await exists('package-lock.json') ? 'Root package lock is present.' : 'package-lock.json must be generated by a trusted Node 20 npm install; do not hand-author it.', ['package-lock.json']);
...
const journeys = ['persistent workspace', 'restores the last saved project', 'answer, health and replay', 'System Check', 'mobile viewport', 'real pasted text', 'malformed project maps', 'Exil rejects'];
const covered = journeys.filter(marker => tests.toLowerCase().includes(marker.toLowerCase()));
add('browser_journey_coverage', covered.length >= 7 ? 'PASS' : 'WARN', `${covered.length}/${journeys.length} essential browser journeys have named coverage.`, ['tests/e2e/neurocanvas.spec.mjs']);
